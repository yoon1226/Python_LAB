...
function setupChat(student) {
  ...
  async function send() {
    ...
    const userContentForAPI = [
      ...
      "요청:",
      "- 전체 코드를 제공하지 마세요.",
      "- 6~10문장 안에서 간결하게 답해주세요.",
      "- 먼저 학생 코드나 질문의 핵심 문제를 1~2문장으로 요약해주세요.",
      "- 이어서 오류 이유나 수정 방향, 또는 아주 짧은 예시(1~3줄)를 제시해주세요.",
      "- 예시를 줄 때는 여러 줄짜리 완성 코드를 쓰지 말고,",
      "  일부를 생략한 형태로만 보여주거나 빈칸을 채울 수 있게 뼈대 코드만 주세요.",
      "- 마지막에는 학생이 스스로 계속 성장할 수 있도록 돕는 질문이나,",
      "  코드를 더 발전시킬 수 있는 방향을 1~2문장으로 제안해주세요."
    ].join("\n");
...
async function requestAiHintOnly({ apiHistory }) {
  ...
  const system = [
    ...
    "정보 과목을 처음 배우는 고등학생도 이해할 수 있을 정도로 쉬운 말을 사용하십시오.",
    "가능하면 어려운 전문 용어 대신 일상적인 표현으로 풀어서 설명하십시오.",
    ...
    " 'if age >= 15: ...' 처럼 뒷부분을 '...'으로 생략해서 제시하십시오.",
    "또는 자연어로 '나이를 입력받은 뒤, if 조건으로 15세 이상인지 비교해 보세요.'처럼 설명만 제공합니다.",

...
function setupChat(student) {
  ...
  async function send() {
    ...
    const userContentForAPI = [
      ...
      "",
      "요청:",
      "- 3~5문장 안에서 간결하게 답해주세요.",
      "- 먼저 학생 코드나 질문의 핵심 문제를 1~2문장으로 요약해주세요.",
      "- 이어서 오류 이유나 수정 방향, 또는 짧은 예시를 제시해주세요.",
      "- 마지막으로 학생이 스스로 계속 성장할 수 있도록 돕는 질문이나, 코드를 더 발전시킬 수 있는 방향을 1~2문장으로 제안해주세요.",
      "- 가장 중요: '현재 단원'에 해당하는 수준까지 이미 배운 문법만 사용해서 예시 코드를 보여주세요.",
       "- 학생이 아직 배우지 않은 문법(예: 반복문 단원에서 함수 정의/호출)은 코드에 사용하지 말고, 필요하면 말로만 '나중에 함수로 묶어볼 수 있다' 정도로 언급해주세요."
    ].join("\n");
